...
namespace dnWalker.TestGenerator.Explorations.Xml
{
    public static class ExplorationDataDeserializer
    {
        ...
        //        (
        //            xml.Element(XmlTokens.XmlBaseParameterSet)?.ToBaseParameterContext() ?? throw new Exception("Exploration iteration XML must have a 'Iteration/StartingState' element."),
        //            xml.Element(XmlTokens.XmlExecutionParameterSet)?.ToExecutionParameterContext() ?? throw new Exception("Exploration iteration XML must have a 'Iteration/EndingState' element."),
