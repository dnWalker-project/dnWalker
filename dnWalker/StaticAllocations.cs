...
namespace MMC.State {
    ...
    public class AllocatedClass : Allocation {
		...
		public void AwakenWaitingThreads() {

			foreach (int sleepy_thread in m_initData.WaitingThreads)
				ActiveState.cur.ThreadPool.Threads[sleepy_thread].Awaken();
		...
		public void AddWaitingThread(int thread_id) {
			if (thread_id < 0)
				System.Console.WriteLine(thread_id);
			ActiveState.cur.ThreadPool.Threads[thread_id].WaitFor(m_initData.InitializingThread);
		...
		private class InitDataContainer : IInitData, IStorable {
			...
			public void Clear() {
				...
				m_isDirty |= m_state != 0 && m_initTrd != LockManager.NoThread && !m_waitingThreads.IsEmpty;
			}
